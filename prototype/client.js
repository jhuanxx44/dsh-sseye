return {
  ...
  apply(ctx) {
    ...
    // Browser-native download via the host export route (no Blob, works in the guarded sandbox).
    ...
    function StepRow(props) {
      ...
      kids.push(h('button', {
        ...
        onClick: (e) => {
          e.stopPropagation()
          download('/__sseye/export?ids=' + encodeURIComponent(it.id) + '&name=' + encodeURIComponent(it.id))
        },
      }, h(DownloadIcon)))
        ...
        h('div', {
          ...
          h('button', {
            ...
            onClick: (e) => {
              ...
              const name = g.kind === 'turn' ? 'turn-' + g.turn : String(g.source || 'group')
              download('/__sseye/export?ids=' + encodeURIComponent(ids) + '&name=' + encodeURIComponent(name))
            },
          }, h(DownloadIcon))),
