	...
	factory: (require) => {
		...
		const dicts = {
			en: {
				...
				"row.hopVia": "Upstream hop: this call was re-dispatched from the wrapping provider \"{via}\", so it shares its parent's turn/step",
				"group.hops": " · {n} upstream hops",
				"group.hops.one": " · {n} upstream hop",
				"hero.hopVia": "via {via}",
				"group.turn": "Turn {n}",
			...
			zh: {
				...
				"row.hopVia": "上游透传：本次调用由包装 provider「{via}」重新派发，与父调用同属一个 turn/step",
				"group.hops": " · {n} 次上游透传",
				"group.hops.one": " · {n} 次上游透传",
				"hero.hopVia": "经由 {via}",
				"group.turn": "Turn {n}",
				"group.compaction": "Compaction",
		...
		*/
		...
		function CloseIcon() {
			return h("svg", {
				...
				strokeLinecap: "round",
				strokeLinejoin: "round"
			}, h("path", { d: "M18 6L6 18" }), h("path", { d: "M6 6l12 12" }));
		}
		...
		*/
		...
		function jsonNode(v, ind, key) {
			...
			if (typeof v === "string") {
				const s = v.length > 4e3 ? v.slice(0, 4e3) + t("json.moreChars", { n: v.length - 4e3 }) : v;
				return h("span", {
					key,
		...
		function BlockContent(props) {
			...
			const type = b.type;
			if (type === "text" && typeof b.text === "string") return copyablePre(cap(b.text, 2e4), "sseye-pre");
			if (type === "reasoning") return copyablePre(cap(typeof b.reasoning === "string" ? b.reasoning : typeof b.text === "string" ? b.text : "", 2e4), "sseye-pre sseye-reason");
		...
		*/
		...
		function StepRow(props) {
			...
			if (depth > 0) cls += " hop";
			const kids = [dot(it.status), depth > 0 ? h("span", {
				key: "s",
				className: "sseye-stepchip sseye-hopchip",
				title: t("row.hopVia", { via: String(it.viaProvider || "") })
			}, "↳") : h("span", {
				key: "s",
				className: "sseye-stepchip"
			}, it.step !== void 0 && it.step !== null ? "S" + it.step : it.source || "")];
			if (depth > 0 && it.provider) kids.push(h("span", {
				key: "pv",
		...
		function TurnGroup(props) {
			...
			const title = g.kind === "turn" ? t("group.turn", { n: String(g.turn) }) : g.source === "compaction" ? t("group.compaction") : g.source === "title" ? t("group.sessionTitle") : t("group.other");
			...
			}, h(Chevron, { open: isOpen }), h("span", { className: "sseye-tgh-title" }, title), h("span", { className: "sseye-chip" }, t("group.calls", { n: steps }) + (hops ? t("group.hops", { n: hops }) : "") + (running ? t("group.running", { n: running }) : "")), h("span", { className: "sseye-tgh-prev" }, prev), h("span", { className: "sseye-tgh-agg" }, "in:" + inTok + " out:" + outTok + " · " + fmtDur(dur)), h("button", {
				className: "sseye-dl",
		...
		function MessageView(props) {
			...
			return h("div", { className: "sseye-msg" + (props.isNew ? " sseye-msg-new" : "") }, h("div", { className: "sseye-msg-head" }, h("span", {
				...
				title: pureToolResult ? t("msg.toolResultRole") : void 0
			}, shownRole), h("span", { className: "sseye-dim" }, t("msg.chars", { n: msgChars(m) }))), body);
		}
		...
		function Hero(props) {
			...
			return h("div", { className: "sseye-hero" }, h("div", { className: "sseye-hero-top" }, dot(d.status), h("span", { className: "sseye-hero-model" }, String(req.provider || "") + "/" + String(req.model || "")), d.protocol ? h("span", {
				...
				title: d.api ? d.api + (d.protocolGuessed ? t("hero.apiGuessed") : t("hero.apiConfigured")) : void 0
			}, (d.protocolGuessed ? "~" : "") + d.protocol) : null, d.source ? h("span", { className: "sseye-chip" }, d.source) : null, d.turn !== void 0 && d.turn !== null ? h("span", { className: "sseye-chip" }, "T" + d.turn + " · S" + d.step) : null, d.parentId ? h("span", {
				className: "sseye-chip sseye-chip-hop",
				title: t("row.hopVia", { via: String(d.viaProvider || "") })
			}, "↳ " + t("hero.hopVia", { via: String(d.viaProvider || "") })) : null, h("span", { className: "sseye-spacer" }), h("button", {
				className: "sseye-btn",
				title: t("row.downloadCall"),
				onClick: () => download(exportUrl(d.id, exportNameOf(d)))
			}, t("hero.download")), h("span", { className: "sseye-dim" }, fmtTime(d.startedAt))), d.baseURL !== void 0 ? h("div", { className: "sseye-hero-ep" }, String(d.baseURL)) : null, h("div", { className: "sseye-stats" }, stats), cacheBar);
		}
		/**
		...
		*/
		const RequestDetail = react.memo(function RequestDetail(props) {
			...
			if (typeof req.system === "string" && req.system) kids.push(h(Section, {
				key: "sys",
				title: t("req.systemPrompt", { n: req.system.length })
			}, copyablePre(cap(req.system, 3e4), "sseye-pre")));
			else if (req.systemOmitted) kids.push(h("div", {
				key: "sys",
				className: "sseye-sec"
			}, h("div", { className: "sseye-sec-title" }, t("req.systemOmitted"))));
			if (Array.isArray(req.messages)) {
				...
				if (shared > 0) msgKids.push(h(Section, {
					key: "shared",
					title: t("req.sharedMessages", { n: shared })
				...
				if (folded > 0) msgKids.push(h(Section, {
					key: "older",
					title: t("req.olderMessages", { n: folded })
				...
				key: "msgs",
				className: "sseye-sec"
			}, h("div", { className: "sseye-sec-title" }, t("req.messagesOmitted", { n: req.messagesOmitted }))));
			if (Array.isArray(req.tools)) {
				const names = req.tools.map((t) => t && t.name);
				kids.push(h(Section, {
					key: "tls",
					title: t("req.tools", { n: req.tools.length })
				}, copyableJson(names)));
			}
		...
		function Detail() {
			...
			if (d.error) kids.push(h("div", {
				key: "err",
				className: "sseye-sec"
			}, h("div", { className: "sseye-sec-title sseye-err" }, t("detail.error")), copyablePre(String(d.error), "sseye-pre sseye-err")));
			...
			if (Array.isArray(d.blocks) && d.blocks.length > 0) kids.push(h("div", {
				key: "resp",
				className: "sseye-sec"
			}, h("div", { className: "sseye-sec-title" }, t("detail.response", { n: d.blocks.length })), d.blocks.map((b) => h(BlockView, {
				key: b.index,
				b
			}))));
		...
		function PolicyPanel() {
			...
			if (!p) return null;
			const srcLabels = [
				["agent", t("policy.src.agent")],
				["compaction", t("policy.src.compaction")],
				["title", t("policy.src.title")],
				["other", t("policy.src.other")]
			];
			const fldLabels = [
				["system", "system"],
				["messages", "messages"],
				["tools", "tools"],
				["reasoning", "reasoning"],
				["text", t("policy.fld.text")],
				["toolArgs", t("policy.fld.toolArgs")]
			];
			const lim = p.limits || {};
			const limLabels = [
				[
					"capacity",
					t("policy.lim.capacity"),
					1,
					5e3,
					10
				],
				...
				[
					"maxBlock",
					t("policy.lim.maxBlock"),
					1e3,
					5e7,
					1e4
				]
			];
			const groupLabel = (s) => s.replace(/[:：]\s*$/, "");
			return h("div", { className: "sseye-policy" }, h("div", { className: "sseye-pgroup" }, h("div", { className: "sseye-plabel" }, groupLabel(t("policy.sources"))), h("div", { className: "sseye-pitems" }, srcLabels.map((kv) => h("label", { key: kv[0] }, h("input", {
				type: "checkbox",
			...
			}), kv[1])))), h("div", { className: "sseye-pgroup" }, h("div", { className: "sseye-plabel" }, groupLabel(t("policy.fields"))), h("div", { className: "sseye-pitems" }, fldLabels.map((kv) => h("label", { key: kv[0] }, h("input", {
				type: "checkbox",
			...
			}), kv[1])))), h("div", { className: "sseye-pgroup" }, h("div", { className: "sseye-plabel" }, groupLabel(t("policy.capacity"))), h("div", { className: "sseye-caps" }, limLabels.map((it) => h("label", {
				key: it[0],
				title: t("policy.limRange", {
					min: it[2],
					max: it[3]
				})
				...
				className: "sseye-note",
				style: { marginTop: "5px" }
			}, t("policy.limitsNote"))), h("textarea", {
		...
		function Panel() {
			...
			return h("div", {
				...
				ref: rootRef
			}, h("div", { className: "sseye-head" }, h("div", { className: "sseye-head-main" }, h("span", { className: "sseye-title" }, "SSEye"), h("span", { className: "sseye-count" }, t("panel.turns", { n: groups.length }) + " · " + t("group.calls", { n: steps }) + (items.length - steps ? t("group.hops", { n: items.length - steps }) : "")), h("span", { className: "sseye-spacer" }), h("button", {
			...
			}, h(CloseIcon))), h("div", { className: "sseye-head-tools" }, h("button", {
				className: "sseye-btn",
			...
			}, store.onlyThisSession ? t("panel.thisSession") : t("panel.all")), h("button", {
				className: "sseye-btn",
				"data-active": store.showPolicy ? "" : void 0,
				onClick: () => {
			...
			}, store.showPolicy ? t("panel.hidePolicy") : t("panel.showPolicy")), h("button", {
				className: "sseye-btn",
				onClick: () => {
					api("/clear", {}).then(() => {
						store.items = [];
			...
			}, t("panel.clear")))), store.showPolicy ? h(PolicyPanel) : null, h("div", { className: "sseye-body" }, h("div", { className: "sseye-listcol" }, groups.length === 0 ? h("div", { className: "sseye-empty" }, t("panel.empty")) : groups.map((g) => h(TurnGroup, {
				key: g.key,
				g
			})))));
		}
